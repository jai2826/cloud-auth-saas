...
app.use("*", cors());
app.use("*", async (c, next) => {
  const sql = src_default(c.env.HYPERDRIVE.connectionString, { max: 5 });
  c.set("sql", sql);
  await next();
});
app.get("/health", async (c) => {
  ...
  return c.json({ status: "ok", db: result[0] });
});
...
// .wrangler/tmp/bundle-WFeg7V/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-WFeg7V/middleware-loader.entry.ts
